...
import { OrderItemService } from 'src/app/shared/services/orderitem.service';
@Component({
...
export class OrdersDetailPageComponent implements OnInit {
  ...
  getItemRentability(item: OrderItem) {
    this._orderItemService.getRentability(item)
      .then(r => {
        item.rentability = r as number;
      })
      .catch(error => {
        alert('Erro ao buscar rentabilidade do produto.');
      });
  }
